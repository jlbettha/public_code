...
def entropy(hist1: np.ndarray[float]) -> float:
    ...
    Args:
        hist1 (np.ndarray[float]): a 1-D histogram or PMF
    ...
    nz_probs = np.array([-p * np.log(p) for p in hist1 if p > TOL])
...
def gini_index(x: np.ndarray[float], w: np.ndarray[float] = None) -> float:
    """
    Betthauser - 2024 - Get gini index, a measure of inequality (e.g., 0.0 means equal distribution).
