...
def gaussian_kernel(size: int, sigma: float = 1.0) -> Tensor:
    kernel = torch.arange(size, dtype=torch.float)
    kernel -= (size - 1) / 2
    kernel = kernel**2 / (2.0 * sigma**2)
...
def channel_conv(
    ...
    """Return the channel-wise convolution of :math:`x` with the kernel `kernel`.
...
def kernel_views(kernel: Tensor, n: int = 2) -> list[Tensor]:
    r"""Return the :math:`N`-dimensional views of the 1-dimensional kernel `kernel`.
...
def channel_convs(
    ...
    r"""Return the channel-wise convolution of :math:`x` with the series of kernel `kernels`.
...
def ssim(
    ...
    r"""Return the SSIM and Contrast Sensitivity (CS) between :math:`x` and :math:`y`.
...
def ms_ssim(
    ...
    r"""Return the MS-SSIM between :math:`x` and :math:`y`.
...
def reduce_tensor(x: Tensor, reduction: str = "mean") -> Tensor:
    r"""Return the reduction of :math:`x`.
...
class SSIM(nn.Module):
    r"""Create a criterion that measures the SSIM between an input and a target.
...
class MS_SSIM(nn.Module):
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        return reduce_tensor(loss, self.reduction)
