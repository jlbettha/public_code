...
def debug(func: Callable) -> Callable:
    """Print the function name, args, kwargs, and result."""
...
def type_enforce(*expected_types: tuple) -> Callable:
    """Enforce the types of the arguments passed to a function."""
...
# decorator
def retry(retries: int = 3, exception: Exception = ValueError, delay: float = 1) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            while attempts < retries:
                try:
                    ...
                    attempts += 1
                    print(f"'{func.__name__}({args}, {kwargs})' attempt {attempts} of {retries}")
                    time.sleep(delay)
            raise exception
...
def record_time(func: Callable) -> Callable:
    """Record the time taken by a function to execute."""
