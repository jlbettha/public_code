...
def main() -> None:
    ...
    while True:
        ...
        b_est[0] = b_est[0] - learning_rate * (
            -2
            * np.sum(
                xs[mini_idx] * (ys_noisy[mini_idx] - xs[mini_idx] * b_est[0] - b_est[1])
            )
            / mini_batch_size
        )
        b_est[1] = b_est[1] - learning_rate * (
            -2
            * np.sum(ys_noisy[mini_idx] - xs[mini_idx] * b_est[0] - b_est[1])
            / mini_batch_size
        )
        sse = (
            ssd(xs[mini_idx] * b_est[0] + b_est[1], ys_noisy[mini_idx])
            / mini_batch_size
        )
    ...
    plt.plot(
        ...
        y_est,
        c="r",
        label=f"Stochastic gradient descent: y={b_est[0]:.2f}x+{b_est[1]:.2f}, {k} iters.",
    )
    plt.legend()
