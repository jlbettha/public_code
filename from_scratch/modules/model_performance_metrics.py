...
class ComputeMetrics:
    ...
    def __init__(self, metric_types=None, overlap=0.1, bg_class=None, n_classes=None):
    ...
    def print_scores(self, metric_types=None):
        ...
        scores = [
            np.mean([self.scores[m][trial] for trial in self.trials])
            for m in metric_types
        ]
        txt = "All: " + " ".join(
            [
                "{}:{:.04}".format(metric_types[i], scores[i])
...
def new_stability(y_true, y_pred):
    true_switch = 0
    pred_switch = 0
    N = y_true.shape[0]
    for i in range(1, N):
        ...
        if y_pred[i] != y_pred[i - 1]:
            pred_switch += 1
    return 1 - (abs(pred_switch - true_switch) / (N - 1))
...
def weighted_accuracy(y_true, y_pred):
    ...
    return sm.accuracy_score(y_true, y_pred, sample_weight=w)


def unweighted_accuracy_occuring_macro(y_true, y_pred, n_classes):
    conf_matrix = sm.confusion_matrix(y_true, y_pred, labels=np.arange(n_classes))
...
def macro_accuracy(P, Y, n_classes, bg_class=None, return_all=False, **kwargs):
    def macro_(P, Y, n_classes=None, bg_class=None, return_all=False):
        ...
        conf_matrix = conf_matrix / (conf_matrix.sum(0)[:, None] + 1e-5)
        conf_matrix = np.nan_to_num(conf_matrix)
        diag = conf_matrix.diagonal() * 1.0  # *100.
    ...
    if type(P) == list:
        ...
        return macro_(
            P, Y, n_classes=n_classes, bg_class=bg_class, return_all=return_all
        )


def acc_per_class(P, Y, bg_class=None, n_classes=None, **kwargs):
    return macro_accuracy(
        P, Y, bg_class=bg_class, return_all=True, n_classes=n_classes, **kwargs
    )[1]
...
def overlap_f1(P, Y, n_classes=0, bg_class=None, overlap=0.1, **kwargs):
    ...
    if type(P) == list:
        return np.mean(
            [overlap_(P[i], Y[i], n_classes, bg_class, overlap) for i in range(len(P))]
        )
    else:
...
def interval_overlap(gt_inter, det_inter):
    ...
    for i in range(gt_inter.shape[0]):
        union = np.maximum(gt_inter[i, 1], det_inter[:, 1]) - np.minimum(
            gt_inter[i, 0], det_inter[:, 0]
        )
        intersection = np.minimum(gt_inter[i, 1], det_inter[:, 1]) - np.maximum(
            gt_inter[i, 0], det_inter[:, 0]
        )
...
def TH14eventdetpr(
    ...
    tp = (conf[1][idx_s] == 1).cumsum() * 1.0
    fp = (conf[1][idx_s] == 2).cumsum() * 1.0
