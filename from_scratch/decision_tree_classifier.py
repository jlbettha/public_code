...
class MyNode:
    ...
    def __init__(self, num_features=None, thr=None, left_child=None, right_child=None, value=None):
...
class MyDecisionTree:
    ...
    def __init__(self, min_split: int = 2, max_depth: int = 15, prune_threshold: float = 0.0):
    ...
    def _grow_next_branch(self, x: np.ndarray[float], y: np.ndarray[float], depth: int = 0):
        """
        _summary_
        ...
        # stopping criteria
        if depth >= self.max_depth or num_labels == 1 or num_datapoints < self.min_split:
        ...
        left = self._grow_next_branch(np.squeeze(x[left_idx, :]), y[left_idx], depth + 1)
        right = self._grow_next_branch(np.squeeze(x[right_idx, :]), y[right_idx], depth + 1)
    ...
    def tree_predict(self, x_test: np.ndarray[float]) -> np.ndarray[int]:
        ...
        Args:
            x_test (np.ndarray[float]): unseen test data
...
def main() -> None:
    ...
    trimmed_tree = MyDecisionTree(max_depth=max_depth, min_split=min_split, prune_threshold=0.1)
