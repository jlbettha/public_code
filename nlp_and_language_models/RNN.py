"""
Created on Sun Jul 16 13:33:33 2017
...
class RNN(object):
    ...
    def __init__(self, insize, outsize, hidsize, learning_rate):
        ...
        self.h = np.zeros(
            (hidsize, 1)
        )  # a [h x 1] hidden state stored from last batch of inputs
    ...
    def train(self, x, y):
        ...
        yhat = (
            {}
        )  # holds 1-of-k representations of predicted y (unnormalized log probs)
        ...
        for t in range(len(x)):
            ...
            h[t] = np.tanh(
                np.dot(self.W_xh, xhat[t]) + np.dot(self.W_hh, h[t - 1]) + self.b_h
            )  # find new hidden state
            yhat[t] = (
                np.dot(self.W_hy, h[t]) + self.b_y
            )  # find unnormalized log probabilities for next chars

            p[t] = np.exp(yhat[t]) / np.sum(
                np.exp(yhat[t])
        ...
        for param, dparam, adaparam in zip(
            [self.W_hh, self.W_xh, self.W_hy, self.b_h, self.b_y],
    ...
    def sample(self, seed, n):
        ...
        for t in range(n):
            h = np.tanh(
                np.dot(self.W_xh, xhat) + np.dot(self.W_hh, h) + self.b_h
            )  # update the state
...
def test():
    ...
    for i in range(len(data) // seq_length):
        x = [
            char_to_ix[c] for c in data[i * seq_length : (i + 1) * seq_length]
        ]  # inputs to the RNN
        y = [
            char_to_ix[c] for c in data[i * seq_length + 1 : (i + 1) * seq_length + 1]
