...
def image_gradient_magnitude(image: NDArray[np.float64]) -> NDArray[np.float64]:
    ...
    img_grads = ndi.gaussian_gradient_magnitude(image, sigma=1.0)
    return img_grads
...
def build_jointhist_n(
    ...
    Args:
        img_colors (_type_): _description_
        img_edges (_type_): _description_
        img_textures (_type_): _description_
        img_grads (_type_): _description_
        img_ranks (_type_): _description_
...
def main() -> None:
    ...
    image = imageio.imread("./my_modules/einstein.jpg")

    image = image_gaussian_smooth(image, sigma=1.0)

    img_grad = image_gradient_magnitude(image)

    img_edgepot = image_edge_potential(image, sigma=1.0)
